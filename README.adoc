...
Writing queries using plain strings — like `"price"` or `"name"` — is fragile. These strings can't be validated by the compiler, so even small typos or refactorings can break your application silently. This is especially risky in dynamic query generation or across large codebases.

Jakarta Data solves this with a **static metamodel**, offering **type-safe**, **fluent**, and **IDE-friendly** access to entity attributes. Unlike Hibernate Criteria APIs that often rely on string-based paths or verbose builders, Jakarta Data metamodels let you write both static and programmatically composed queries that are validated at compile time.
...
- **Fluent Query Construction:** The metamodel provides a fluent API for building both static and dynamic queries. You can define reusable query fragments, conditionally apply filters, and construct criteria programmatically — all without sacrificing type safety.

...
The entity annotation class will usually be sufficient to avoid conflicts between Jakarta Data providers. Still, in cases where the entity annotation class is insufficient, the application can designate the name of the desired Jakarta Data provider on the optional `provider` attribute of the `Repository` annotation.

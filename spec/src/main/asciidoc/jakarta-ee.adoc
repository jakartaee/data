...
In the Jakarta Data specification, when integrating with Jakarta Bean Validation, the validation process should occur during the insert and update operations, specifically within the save methods of the Repositories interfaces. It means that when performing an insert or update operation on a database, Jakarta Bean Validation will execute the defined constraints to validate the data being inserted or updated.

...
TODO: in a subsequent PR, determine if any parts of the following need to remain in Jakarta Data. Otherwise, content that is duplicated by Jakarta Query can be removed.

=== Type system

Every expression in a JCQL query is assigned a Java type. An implementation of JCQL is required to support the Java types listed in <<Basic types>>, that is: primitive types, `String`, `LocalDate`, `LocalDateTime`, `LocalTime`, `Year`, and `Instant`, `java.util.UUID`, `java.math.BigInteger` and `java.math.BigDecimal`, `byte[]`, and  `enum` types.

NOTE: An implementation of JCQL is permitted and encouraged to support additional types. Use of such types is not guaranteed to be portable between implementations.
...
Since an attribute of an entity may be null, a JCQL expression may evaluate to a null value.
...
In the JCQL grammar, identifiers are labelled with the `IDENTIFIER` token type.
...
NOTE: Use of a reserved identifier as a regular identifier in JCQL might be accepted by a given Jakarta Data provider, but such usage is not guaranteed to be portable between providers.
...
NOTE: JCQL does not require support for literals written in octal or hexadecimal.
...
An expression is a sequence of tokens to which a Java type can be assigned, and which evaluates to a well-defined value when the query is executed. In JCQL, expressions may be categorized as:
...
A _function call_ is the name of a JCQL function, followed by a parenthesized list of argument expressions, with syntax given by `function_expression`.
...
For numeric types, and for date, time, and datetime types, the total order is unique and completely determined by the semantics of the type. JCQL implementations must sort these types according to their natural order, that is, the order in JCQL must agree with the order defined by Java.
...
- Textual data is represented in JCQL as the type `java.lang.String`. Strings are in general ordered lexicographically, but the ordering also depends on the character set and collation used by the database server. Applications must not assume that the order agrees with the `compareTo()` method of `java.lang.String`. In evaluating an inequality involving string operands, an implementation of JCQL is not required to emulate Java collation.

- Binary data is represented in JCQL as the type `byte[]`. Binary data is in general ordered lexicographically with respect to the constituent bytes. However, since this ordering is rarely meaningful, this specification does not require implementations of JCQL to respect it.
...
CAUTION: A compliant implementation of JCQL might feature SQL/JPQL-style ternary logic, where `not n > 0` is an unsatisfied logical expression when `n` evaluates to null, or it might feature binary logic where the same expression is considered satisfied. Application programmers should take great care when using the `not` operator with scalar expressions involving `null` values.
...
Each JCQL statement is built from a sequence of _clauses_. The beginning of a clause is identified by a keyword: `from`, `where`, `select`, `set`, or `order`.
...
The Jakarta Query specification defines the syntax of JCQL.

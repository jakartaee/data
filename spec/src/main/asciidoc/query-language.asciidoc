...
Due to the _eventual persistence_ behavior common in many NoSQL databases, returning the number of updated records as an `int` or `long` might not be supported. Even when supported, the value may be imprecise, reflecting an estimate or partial result rather than an exact count.
...
Due to the _eventual persistence_ behavior common in many NoSQL databases, even when a delete operation appears to succeed, the underlying system may not provide an accurate or immediate count of affected records. As a result, the returned count may be imprecise or delayed.

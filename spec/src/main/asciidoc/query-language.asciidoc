...
NOTE: When a `select` clause contains more than one path expression, the `@Select` annotation is used to map the elements of the tuple to constructor parameters of the query return type.
When the `select` clause contains only one path expression, the `@Select` annotation is not used, and the query directly returns the values of the path expression.

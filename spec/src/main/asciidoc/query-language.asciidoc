...
Key-Value Databases:: Support for a `WHERE` clause that performs an equality restriction on the key attribute is required. Key-value databases might not be capable of queries without a `WHERE` clause or with a `WHERE` clause that does not include an equality restriction on the key attribute.

Wide-Column Databases:: Support for a `WHERE` clause that performs an equality restriction on the key attribute is required. Wide-column databases might not be capable of queries without a `WHERE` clause or with a `WHERE` clause that does not include an equality restriction on the key attribute.

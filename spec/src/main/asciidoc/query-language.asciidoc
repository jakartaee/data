...
If a NoSQL provider does not support the execution of conditional deletes or cannot determine the number of deleted records, a `delete` operation that returns an `int` or `long` must throw an `UnsupportedOperationException`.
====

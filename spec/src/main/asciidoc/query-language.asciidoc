...
[WARNING]
====
Support for update statements may vary across NoSQL databases.

In particular:

- **Key-Value databases** typically only support updates by key. They do not allow conditional updates using a `where` clause, and returning the number of updated records is generally unsupported.
- **Wide-Column databases** may restrict updates to records identified by primary or partition keys. Conditional updates or returning update counts may not be supported unless secondary indexes are used.
- **Document and Graph databases** are more likely to support conditional updates and return the number of updated records. However, the semantics and consistency guarantees may vary depending on the provider.

Due to the **eventual persistence** behavior common in many NoSQL databases, returning the number of updated records as an `int` or `long` might not be supported. Even when supported, the value may be **imprecise**, reflecting an estimate or partial result rather than an exact count.

Additionally, in databases with **append-only semantics**—such as many time-series and wide-column databases—the `update` operation may behave more like an `insert`, and repeated updates to the same record may not overwrite previous values.

In such cases, the Jakarta Data provider must throw an `UnsupportedOperationException` when a repository method declares an `int` or `long` return type for an update query that the data store cannot fulfill accurately.
====
...
[WARNING]
====
Support for delete statements may vary significantly across NoSQL databases.

In general:

- **Key-Value databases** often only support delete operations by key. Queries with complex `where` clauses may not be supported, and returning the count of deleted records is typically not possible.
- **Wide-Column databases** may have limited support for deletions with conditions beyond the primary key or partition key. Additionally, support for returning a count of deleted records may require the use of secondary indexes or may be entirely unsupported.
- **Document and Graph databases** are more likely to support conditional deletes. However, the behavior and performance characteristics (especially around consistency and the returned delete count) can differ between providers.

Due to the **eventual persistence** behavior that is common in many NoSQL databases, even when a delete operation appears to succeed, the underlying system may not provide an accurate or immediate count of affected records. As a result, the returned count may be **imprecise** or delayed.

If a NoSQL provider does not support the execution of conditional deletes or cannot determine the number of deleted records reliably, a `delete` operation that returns an `int` or `long` must throw an `UnsupportedOperationException`.
====

...
/**
 * <p>A constraint that requires a {@code true} value.</p>
 *
 ...
 * {@code True}. For example,</p>
 *
 * <pre>{@code
 * @Find
 * List<Car> usedVehicles(@By(_Car.MAKE) String make,
 *                        @By(_Car.MODEL) String model,
 *                        @By(_Car.PREVIOUSLYOWNED) True preOwned);
 * ...
 *
 * found = cars.usedVehicles("Jakarta Motors",
 *                           "Jukon",
 *                           True.instance());
 * }</pre>
 *
 * <p>Repository methods can also accept {@code True} constraints at run time
 ...
 * @since 1.1
 */
public interface True extends Constraint<Boolean> {

    /**
     * <p>Requires that the constraint target has a {@code true} value. For
     * example,</p>
     *
     * <pre>{@code
     *     found = cars.preOwnedVehicles("Jakarta Motors",
     *                                   "Jukon",
     *                                   True.instance());
     * }</pre>
     *
     * @return a {@code True} constraint.
     */
    static True instance() {
        return TrueRecord.INSTANCE;
    }
}

...
/**
 * <p>A built-in repository supertype for performing Create, Read, Update, and
 * Delete (CRUD) operations.</p>
 *
 * <p>This repository extends the {@link BasicRepository} interface, adding
 * {@linkplain #insert} and {@linkplain #update}
 * operations in addition to basic {@linkplain #findById retrieval} and
 * {@linkplain #delete deletion}. This interface combines the Data Access Object
 * (DAO) aspect with the repository pattern, offering a versatile and complete
 * solution for managing persistent entities within Java applications.</p>
 *
 * <p>The type parameters of {@code CrudRepository<T,K>} capture the primary
 * entity type ({@code T}) for the repository
 * and the type of the unique identifier attribute ({@code K}) of the primary
 * entity type.</p>
 *
...
public interface CrudRepository<T, K> extends BasicRepository<T, K> {
     ...
     * written to the database, including all automatically generated values and
     * incremented values that changed due to the insert. After invoking this
     * method, do not continue to use the instance that is supplied as a
     * parameter. This method makes no guarantees about the state of the
     ...
     * written to the database, including all automatically generated values and
     * incremented values that changed due to the insert. After invoking this
     * method, do not continue to use the entity instances that are supplied in
     * the parameter. This method makes no guarantees about the state of the
     * entity instances that are supplied in the parameter. The position of
     * entities within the {@code Iterable} return value must correspond to the
     * position of entities in the parameter based on the unique identifier of
     * the entity.</p>
     *
     ...
     *
     * <p>For an update to be made, a matching entity with the same unique
     * identifier
     ...
     * {@link #insert} method.</p>
     *
     * <p>If the entity is versioned (for example, with
     * {@code jakarta.persistence.Version} or by
     * another convention from the entity model such as having an attribute
     * named {@code version}), then the version must also match. The version is
     * automatically incremented when making the update.</p>
     *
     ...
     *
     * <p>For an update to be made to an entity, a matching entity with the same
     * unique identifier
     * must be present in the database. In databases that use an append model to
     * write data or follow the BASE model, this method behaves the same as the
     * {@link #insertAll} method.</p>
     *
     * <p>If the entity is versioned (for example, with
     * {@code jakarta.persistence.Version} or by
     * another convention from the entity model such as having an attribute
     ...
     * @return updated entity instances, in the same order as the supplied
     * entities, and including all automatically generated values, updated
     * versions, and incremented values which changed as a result of the
     * update.
     * @throws OptimisticLockingFailureException If any of the supplied entities
     *                                           is not found in the database or
     *                                           has a version that differs from
